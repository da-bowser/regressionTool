...
public class IntegratedConfiguration extends IntegratedConfigurationMain {
	...
	public void startExtraction() {
		...
		try {
			...
			correctMessageMappingFile(responseBytes);
	...
	/**
	 * Find Parent ID in response XML (GetMessageList response) and replaced it with its Message Id.
	...
	private void correctMessageMappingFile(InputStream responseBytes) throws ExtractorException {
        ...
        // Extract Message ID's from web service response (Parent and Message ID)
        HashMap<String, String> updatedMessageIds = findMessageIdsToBeReplaced(responseBytes);

        // Modify relevant, existing entries in Message Mapping file
        updateMessageIdMappingFile(updatedMessageIds);
	}
	...
	private HashMap<String, String> findMessageIdsToBeReplaced(InputStream responseBytes) throws ExtractorException {
		final String SIGNATURE = "findMessageIdsToBeReplaced(InputStream)";
		try {
			...
			while (eventReader.hasNext()) {
				...
				case XMLStreamConstants.END_ELEMENT:
					...
					if ("parentID".equals(currentEndElementName)) {
						splitMessageIds.put(parentId, messageId);
					}
					break;
				}
			}
			...
			String msg = "Error extracting combinations of Parent and Message Ids.\n" + e.getMessage();
