...
class IntegratedConfigurationTest {
	...
	void extractMessageInfoFiltersCorrect() {
		try {
			...
			try (InputStream is = new FileInputStream(new File(pathResponse))) {				
				...
				MessageInfo msgInfo = IntegratedConfiguration.extractMessageInfo(is.readAllBytes(), "ia_CrossApplicationLogging");
	...
	void checkBuildingOfMessageIdListWorks() {
		try {
			...
			try (InputStream is = new FileInputStream(new File(pathResponse))) {
				// Extract data from response
				MessageInfo msgInfo = IntegratedConfiguration.extractMessageInfo(is.readAllBytes(), "Data_In_Async_Split");
	...
	void checkExtractMessageInfo() {
		try {
			...
			try (InputStream is = new FileInputStream(new File(pathResponse))) {			
				// Extract data from response
				MessageInfo msgInfo = IntegratedConfiguration.extractMessageInfo(is.readAllBytes(), "ia_PackingList_KOL");
